...
void __attribute__((weak)) ble_stack_init (void) {
    ...
    if(err_code != 0){
        //Count number of blinks to determine error code number
        for(unsigned int i = 1; i <= err_code; i++){
            led_toggle(LED);
            nrf_delay_ms(1000);
            led_toggle(LED);
            nrf_delay_ms(1000);
    ...
    //NOTE: after testing, LED blinks 4 times - the problem is nrf_error_no_mem
    APP_ERROR_CHECK(err_code);
